...
**Alternative 1**: If the server runs on some domain (for example `mydomain.org`) a certificate can be automatically issued and installed by using a service like `Lets Encrypt <https://en.wikipedia.org/wiki/Let%27s_Encrypt>`_, ZeroSSL or similar... along with a *certbot*, a certificate is placed at a certain location: You may check if the directory `/etc/letsencrypt/live/` exists.
