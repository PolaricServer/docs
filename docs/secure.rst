...
The default setup is to use HTTPS between the client and the server and to use a frontend-webserver (Apache) as a *proxy* for the backend (with a proper URL prefix). HTTPS is handled by the frontend. The backend uses a separate port (default is port 8081). HTTPS is must be used for login-sessions to other servers than localhost. 

.. note::
    In version 4.2 the *backend* doesn't support HTTPS and the ``httpserver.secure`` will have no other effect than announcing it over mDNS. The frontend should always be used if client use it from different machines than where the server runs. We may consider re-implementing HTTPS in the backend in later versions, if there is a specific need. 
...
**Alternative 0**: **This is the default setting in Polaric Server 3.0 and later.** A `self-signed <https://en.wikipedia.org/wiki/Self-signed_certificate>`_ certicate is generated by the package *ssl-cert* when installing the server. This certificate can be used by *Polaric Aprsd*, but clients browsers need to be told explicitly that this certificate is ok to use (make an exception from the general rule not to trust unverified certificates). The frontend (Polaric Webapp2) is also configured to use this certificate for secure connections. Client web-browsers should use HTTPS. To configure *aprsd* to use this certificate use this command (as root)::
